...
public class TencentSmsMfaChannelAdapter implements MfaChannelAdapter {
    ...
    public MfaChannelDescriptor descriptor() {
        SsoProperties.Mfa.TencentSmsConfig cfg = ssoProperties.getMfa().getChannels().getTencentSms();
        return new MfaChannelDescriptor(
                channelKey(), "腾讯云短信", "登录 step-up 短信验证码（可选）",
    ...
    public boolean isEnabled() {
        return ssoProperties.getModules().isSmsMfa()
                && ssoProperties.getMfa().getChannels().getTencentSms().isEnabled()
                && credentialsReady(ssoProperties.getMfa().getChannels().getTencentSms());
    }

    @Override
    public String sendChallenge(SsoUser user, String challengeToken) {
        if (!isEnabled()) {
            throw new SsoException(ErrorCode.BAD_REQUEST, "腾讯云短信 MFA 未启用");
        }
        ...
        log.info("[TencentSMS-MFA] to={} code={} (配置 SDK 后改为真实发送)", maskPhone(phone), code);
    ...
    private boolean credentialsReady(SsoProperties.Mfa.TencentSmsConfig cfg) {
        return StringUtils.hasText(cfg.getSecretId())
                && StringUtils.hasText(cfg.getSecretKey())
                && StringUtils.hasText(cfg.getSdkAppId())
                && StringUtils.hasText(cfg.getTemplateId())
